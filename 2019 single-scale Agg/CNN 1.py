...
def build_rand_feat():
    ...
    for _ in tqdm(range(n_samples)):
        ...
        wav, rate = sf.read('clean2019/' + file)
        label = df.at[file, 'label']
        rand_index = np.random.randint(0, wav.shape[0] - config.step)
        sample = wav[rand_index:rand_index + config.step]

        X_sample = mfcc(sample, rate, winlen=0.02, winstep=0.02, numcep=config.nfeat, nfilt=config.nfilt,
                        nfft=config.nfft)
        # print(X_sample)
...
def get_conv_model():
    ...
    main_model = Sequential()
    main_model.add(MaxPool2D((2, 2), padding='same', input_shape=input_shape))
    ...
    main_model.add(BatchNormalization())
    main_model.add(Dropout(0.3))
    main_model.add(Dense(15, activation="softmax"))
    main_model.compile(loss="categorical_crossentropy", optimizer=Adam(0.001), metrics=['acc'])
    print(K.eval(main_model.optimizer.lr))
    main_model.summary()
...
for f in df.index:
    ...
    signal, rate = sf.read('clean2019/' + f)
...
model.fit(X, y, epochs=100, batch_size=64, shuffle=True, validation_split=0.1, class_weight=class_weight,
